...
contract TokenizedVickeryAuction {
    ...
    error TVA__AUCTION_HAS_UNREVEALED_BIDS_AND_IS_IN_REVEAL_PERIOD(uint32 endTime);
    ...
    event AuctionEnded(
        address indexed tokenContract, uint256 indexed tokenId, address indexed winner, uint96 winningBid
    );
    ...
    function createAuction(
        ...
        _createAuction(tokenContract, tokenId, erc20Token, startTime, bidPeriod, revealPeriod, reservePrice);
    }
    ...
    function _createAuction(
        ...
        if (auction.status) {
            revert TVA__AUCTION_EXISTS(auction.startTime, auction.endOfRevealPeriod);
        }
        ...
        emit AuctionCreated(
            tokenContract, tokenId, msg.sender, startTime, startTime + bidPeriod, reservePrice, index, erc20Token
        );
    }
    ...
    function commitBid(address tokenContract, uint256 tokenId, bytes20 commitment, uint256 erc20Tokens) external {
        ...
        bids[tokenContract][tokenId][auctionIndex][msg.sender] = Bid({commitment: commitment, collateral: collateral});

        auction.numUnrevealedBids += 1;

        emit BidCommitted(tokenContract, tokenId, msg.sender, auctionIndex, collateral);
    }
    ...
    function revealBid(address tokenContract, uint256 tokenId, uint96 bidValue, bytes32 nonce) external {
        ...
        bytes20 commitment = bytes20(keccak256(abi.encode(nonce, bidValue, tokenContract, tokenId, auctionIndex)));
        ...
        emit BidRevealed(tokenContract, tokenId, msg.sender, auctionIndex, bidValue);
    }
    ...
    function endAuction(address tokenContract, uint256 tokenId) external {
        ...
        if (block.timestamp < auction.endOfRevealPeriod) {
            if (auction.numUnrevealedBids > 0) {
                revert TVA__AUCTION_HAS_UNREVEALED_BIDS_AND_IS_IN_REVEAL_PERIOD(auction.endOfRevealPeriod);
            }
        ...
        Bid storage bid = bids[tokenContract][tokenId][auctionIndex][highestBidder];
    ...
    function withdrawCollateral(address tokenContract, uint256 tokenId, uint64 auctionIndex) external {
        ...
        emit CollateralWithdrawn(tokenContract, tokenId, msg.sender, auctionIndex, bid.collateral);
    }
